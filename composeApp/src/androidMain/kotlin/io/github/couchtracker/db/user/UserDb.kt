...
sealed class UserDb {
     ...
     * @see write
     */
    suspend fun <T> transaction(context: Context, block: DatabaseTransaction<TransactionResult<T>>): UserDbResult<T> {
    ...
    companion object {
            ...
            val driver = AndroidSqliteDriver(
                ...
                callback = object : AndroidSqliteDriver.Callback(UserData.Schema) {
                    override fun onCorruption(db: SupportSQLiteDatabase) {
                        Log.e(LOG_TAG, "Database is corrupted!")
         ...
         */
        @JvmStatic
        protected fun Uri.copyToInternal(appContext: Context, file: File): UserDbResult<Nothing>? {
         ...
         */
        @JvmStatic
        protected fun File.copyToExternal(appContext: Context, uri: Uri): UserDbResult<Nothing>? {
        ...
        /**
         * Converts any positive value to an [Instant], assuming epoch milliseconds. Returns null otherwise.
         */
        @JvmStatic
        protected fun Long.toInstantAsEpochMillisecondsOrNull(): Instant? {
            return takeIf { it > 0 }?.let { Instant.fromEpochMilliseconds(it) }
        }
    }
}
