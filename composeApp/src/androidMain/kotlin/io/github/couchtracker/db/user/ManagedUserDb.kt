...
class ManagedUserDb(
     ...
     */
    suspend fun moveToExternalDb(context: Context, uri: Uri): UserDbResult<Unit> {
        return withContext(Dispatchers.IO) {
            ...
            managedDbPath.file.copyToExternal(context, uri)?.let { return@withContext it }
            ...
            appData.userQueries.setExternalFileUri(
                id = user.id,
                externalFileUri = uri,
                cachedDbLastModified = externalDocument.lastModified().toInstantAsEpochMillisecondsOrNull(),
            )
    ...
    override suspend fun unlink(context: Context) {
        ...
        if (dbPath.file.exists()) {
            Log.d(LOG_TAG, "Deleting internal DB $this")
            dbPath.file.delete()
        }
    }
