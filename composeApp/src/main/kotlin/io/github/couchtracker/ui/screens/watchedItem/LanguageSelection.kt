...
fun WatchedItemSheetScope.LanguageSection(
    ...
    val suggestedLanguages = mediaLanguages
        ...
        .plus(data.topUsedLanguages(selection.dimension).take(SUGGESTED_LANGAUGES_TOP_USED_LIMIT))
        .distinct()
 ...
 */
private fun FullProfileData.topUsedLanguages(dimension: WatchedItemDimensionWrapper.Language): List<Bcp47Language> {
    return this.watchedItems
        // Filter items that have been added recently
        .filter { Clock.System.now() - it.addedAt < USED_LANGUAGES_RECENCY_CUTOFF }
