...
fun Duration.remainderUntilNextUnitBoundary(unit: DurationUnit): Duration {
    ...
    return if (isPositive()) {
        when (remainderNs) {
            durationNanos -> remainderNs.nanoseconds + Duration.ZERO.remainderUntilNextUnitBoundary(unit)
            else -> (remainderNs + 1).nanoseconds
        }
    } else {
        when (remainderNs) {
            0L -> unitNanos
            else -> unitNanos - remainderNs
        }.nanoseconds
    }
}


fun DurationUnit.toImbIcuTimeUnit(): MeasureUnit = when (this) {
